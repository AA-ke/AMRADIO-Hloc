...
def compute_cosine_similarity(desc1, desc2):
    """计算描述符之间的余弦相似性"""
    norm_desc1 = desc1 / (np.linalg.norm(desc1, axis=1, keepdims=True) + 1e-8)
    norm_desc2 = desc2 / (np.linalg.norm(desc2, axis=1, keepdims=True) + 1e-8)
    similarity = np.dot(norm_desc1, norm_desc2.T)  # (L1, L2)
    return similarity

# 用余弦相似性代替距离
def match_with_cosine_similarity(desc1, desc2, thresh=1):
    similarity = compute_cosine_similarity(desc1, desc2)
    nearest = np.argpartition(similarity, -2, axis=-1)[:, -2:]  # 找到相似度最高的两个
    sim_nearest = np.take_along_axis(similarity, nearest, axis=-1)
    valid_mask = sim_nearest[:, -1] >= thresh * sim_nearest[:, -2]
    matches = np.stack([np.where(valid_mask)[0], nearest[valid_mask][:, -1]], axis=1)
    return matches
...
# 加载图片
img1 = Image.open('examples/north_000021.jpg').convert("RGB")
img2 = Image.open('examples/qie2_0021.jpg').convert("RGB")
...
# 调用
matches = match_with_cosine_similarity(desc1, desc2, thresh=0.5)
